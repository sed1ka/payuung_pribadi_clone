...
import 'package:payuung_pribadi_clone/features/personal_information/company/pendapatan_kotor_per_tahun/pendapatan_kotor_per_tahun_bottomsheet.dart';
import 'package:payuung_pribadi_clone/utils/formatter.dart';
import 'package:payuung_pribadi_clone/utils/page_navigator.dart';


class PendapatanKotorPerTahunSelector extends StatelessWidget {
  const PendapatanKotorPerTahunSelector({
    super.key,
  ...
  Future<void> selectData(
    BuildContext context,
      PendapatanKotorPerTahunBloc bloc,
  ) async {
    final dynamic res = await showKreduitModalBottom(
      ...
      builder: (_) => BlocProvider.value(
        value: bloc,
        child: const PendapatanKotorPerTahunBottomSheet(),
      ),
    );
  ...
  Widget build(BuildContext context) {
    final PendapatanKotorPerTahunBloc bloc = BlocProvider.of(context);
    ...
    return BlocConsumer(
      ...
      builder: (_, __) => DataSelectorTextField(
        ...
        onTap: () => bloc.add(OpenSelectorPendapatanKotorPerTahun()),
      ),
    );
  }
}
